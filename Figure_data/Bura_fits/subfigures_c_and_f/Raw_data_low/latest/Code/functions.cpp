...
void check_reaction_table_distribution(const std::vector<TList>& Table_cellu, const std::vector<TList>& Table_hemi,params& par){
    double a0 = 0;
...
void lignin_glue(params& par, vector<TList>& Table_cellu, vector<TList>& Table_hemi, TList& Table_lign, vector<float>& chem_entities, int nbr_poly_lign){
    ...
    if(test1 == 1){
        ...
        if(test <= chem_entities[0])
            enzyme_number = 1;
        else if(test <= chem_entities[1])
            enzyme_number = 2;
        else if(test <= chem_entities[2])
            enzyme_number = 3;
    ...
    int lignins_glued = int(drand48() * (2.*float(par.enzyme_radius) + 1.));//the enzyme blocks up to 2*r_enzyme + 1 lignin bonds
    if(lignins_glued > 0)
...
/*
    int r_check = int(par.enzyme_radius);//Distance over which each direction is investigated
    if (r_check <= 0)
        r_check = 1;

    int N_checks = 0;//Number of coordinate sets investigated
    int N_stops = 0;//Number of times the following ray-based check encountered a bond
    int vec_x,vec_y,vec_z; //Components of "unit" vector
    int pos_x,pos_y,pos_z; //Position to be checked for existing bonds
    vec_x = -1;
    vec_y = -1;
    vec_z = -1;

    while(vec_x <= 1 or vec_y < 1 or vec_z < 1){
        vec_y = -1;
        while(vec_y <= 1){
            vec_z = -1;
            while(vec_z <= 1){
                ...
                pos_z = vec_z;
                N_checks++;
                for(int i=0;i<r_check;i++){
                    if(bond_exists(celluList, hemiList, lignList,pos_x,pos_y,pos_z) == true){
                        N_stops++;
                        break;
                    }
                    pos_x++;
