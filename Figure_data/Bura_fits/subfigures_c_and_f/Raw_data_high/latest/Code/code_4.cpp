      	...
      	if(output_bool == true){
		    if(par.time_mean.size() > 0){
	            ...
	            if(par.time_mean.size() > 0){
	    	        ...
	    	        EnzymeFile << par.time_mean[0] << "\t" << par.EG_activity[0] << "\t" << par.CBH_activity[0] << "\t" << par.BGL_activity[0] << "\t" << par.XYL_activity[0] << endl;
	    	        for(int i=1;i<par.time_mean.size();i++){
	    	        	EnzymeFile << par.time_mean[i] << "\t" << par.EG_activity[i] << "\t" << par.CBH_activity[i] << "\t" << par.BGL_activity[i] << "\t" << par.XYL_activity[i] << endl;
	    	        }
	     			EnzymeFile.close();
	            }
	            memset(par.output_file, 0, sizeof(par.output_file));
	            ...
	            if(par.timestamp.size()>0){
	                ...
	                EnzymeFractionFile << par.timestamp[0] << "\t" << par.EG_fraction[0] << "\t" << par.CBH_fraction[0] << "\t" << par.BGL_fraction[0] << "\t" << par.XYL_fraction[0] << endl;
	                for(int i=1;i<par.timestamp.size();i++){
	                    EnzymeFractionFile << par.timestamp[i] << "\t" << par.EG_fraction[i] << "\t" << par.CBH_fraction[i] << "\t" << par.BGL_fraction[i] << "\t" << par.XYL_fraction[i] << endl;
	                }
...
//        if(verbose == true)
 ...
 //       if(verbose == true)
    ...
    if((par.mode_code==3) or (par.mode_code==4))
        nbr_poly_cellu=18;
...
//                            cout << "(" << cellu[i].x << ", " << cellu[i].y << ", " << cellu[i].z[j] << "): " << bond_neighbors_cellu[current_key].N_neighbors << endl;
    ...
    if(int(t0) % enzyme_activity_Freq == 0)
        check_reaction_table_distribution(Table_cellu,Table_hemi,par);
